...
void NANOFLANNWrapper2D::CreatePointKDTree()
{
	// how many point?
...
void NANOFLANNWrapper2D::CreatePointWithInfoKDTree()
{
	// how many point?
	int numPt = _pointData.size();
...
void NANOFLANNWrapper2D::SetPointData(std::vector<A2DVector> myData)
{
	this->_pointData = myData;
}

void NANOFLANNWrapper2D::SetPointDataWithInfo(std::vector<A2DVector> myData, std::vector<int> info1, std::vector<int> info2)
{
	this->_pointData = myData;
...
void NANOFLANNWrapper2D::AppendPointData(std::vector<A2DVector> myData)
{
...
std::vector<A2DVector> NANOFLANNWrapper2D::GetClosestPoints(A2DVector pt, int num_query)
{
	...
	std::vector<A2DVector> retVectors;

	for (int a = 0; a < num_query; a++)
	{
		A2DVector retPt(_pointCloud.pts[ret_index[a]].x, _pointCloud.pts[ret_index[a]].y);
...
std::vector<int> NANOFLANNWrapper2D::GetClosestIndices(A2DVector pt, int num_query)
{
	float query_pt[2] = { pt.x, pt.y };
	std::vector<size_t> ret_index(num_query);
	std::vector<float> out_dist_sqr(num_query);
...
std::vector<std::pair<int, int>> NANOFLANNWrapper2D::GetClosestPairIndices(A2DVector pt, int num_query)
{
	float query_pt[2] = { pt.x, pt.y };
	std::vector<size_t> ret_index(    num_query);
	std::vector<float>  out_dist_sqr( num_query);
