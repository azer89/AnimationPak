...
public:
	...
	A2DVector GetPolygonCentroid(const std::vector<A2DVector>& contour);

	// ---------- Get contour ----------
	std::vector<A2DVector> GetContour(CVImg img);

	// ---------- Get contours ----------
	std::vector<std::vector<A2DVector>> GetContours(CVImg img, int blurSize = 0);
	...
	// ---------- signed distance ----------
	CVImg CalculateSignedDistance(const std::vector<A2DVector>& contour);

	// ---------- signed distance ----------
	CVImg CalculateSignedDistance(const std::vector<A2DVector>& contour, CVImg aMask);
	...
	// ---------- conves hull ----------
	std::vector<A2DVector> GetConvexHull(std::vector<A2DVector> polygon);

	float GetArea(const std::vector<A2DVector>& polygon)
	{
		std::vector<cv::Point2f> cvPoly = ConvertList<A2DVector, cv::Point2f>(polygon);
		return cv::contourArea(cvPoly);

	}

	float GetAreaOriented(const std::vector<A2DVector>& polygon)
	{
		std::vector<cv::Point2f> cvPoly = ConvertList<A2DVector, cv::Point2f>(polygon);
	...
	// ---------- text ----------
	void PutText(std::string imageName, std::string text, A2DVector pos, MyColor col, float scale = 0.5f, float thickness = 1);
	void PutText(cv::Mat img, std::string text, A2DVector pos, MyColor col, float scale = 0.5f, float thickness = 1);
	...
	void DrawCircle(cv::Mat img, A2DVector pt, MyColor col, float radius)
	{
	...
	/*CVImg GetDistanceMapSlow(int numRegion,
		...
		const std::vector<std::vector<A2DVector>>& focalBoundaries);*/

	// ---------- from ShapeRadiusMatching ----------
	std::vector<std::vector<cv::Point>> GetCVContours(const std::vector<std::vector<A2DVector>>& contours);


	// ---------- for Dijkstra ----------
	/*CVImg CreateDijkstraMask(int blobNumber,
		std::vector<A2DVector> combinedBoundary,
